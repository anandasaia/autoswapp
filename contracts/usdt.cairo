...
// Copyright (C) 2021 usdt Foundation
...
func mint{
   ...
   // auth();
    ...
    with_attr error_message("usdt/invalid-recipient") {
    ...
    // check valid amount
    with_attr error_message("usdt/invalid-amount") {
...
func burn{
    ...
    with_attr error_message("usdt/insufficient-balance") {
    ...
    if (caller != account) {
        ...
        if (eq == 0) {
            with_attr error_message("usdt/insufficient-allowance") {
...
func auth{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ...
    with_attr error_message("usdt/not-authorized") {
        assert ward = 1;
    }
...
func uint256_add_safe{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("usdt/uint256-overflow") {
        assert carry = 0;
    }
    return (sum,);
}
